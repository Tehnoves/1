...
ROM char InitConfigRegsPri[] = {
		...
		/* 5 */				0b01001111,                                       //0b00001111 FIFOSIZE_64 | FIFO_THRSHOLD_1
		/* 6 */				153,
		/* 7 */				122,
		/* 8 */				68,
		/* 9 */				0,
		...
		ROM	const char  InitConfigRegsPer[] = {
		/* 0 */				CHIPMODE_STBYMODE | FREQBAND_915 | VCO_TRIM_00,   //0b00110000
		/* 1 */				MODSEL_OOK | DATAMODE_PACKET | IFGAIN_0 | DATAMODE_BUFFERED | OOKTHRESHTYPE_PEAK,          //0b01101100 
...
unsigned char delay;
unsigned char  A2,A3;
unsigned char  command,val,RxPacketLen,diagnoz[12];
...
void Send_Packet_my(void)
	{
		...
		write_spi_con(0x1F, ((InitConfigRegsPer[0x1F] & 0xBF)| FIFO_STBY_ACCESS_WRITE));// FIFO_STBY_ACCESS_READ
	a1 =read_spi_con(0x1f); 
		...
		write_spi_con(0x16, 0x97);	//на передачу     0x97
		dia();
	...
	unsigned char ReceiveFrame_my(void)
		{
			unsigned char  dat, node_adrs, a;
			unsigned char i = 0;
			write_spi_con(0x1F, ((InitConfigRegsPri[0x1F] & 0xBF)| FIFO_STBY_ACCESS_READ));// 
...
__delay_ms(5);
	...
	*/
	...
	SSP2STATbits.SMP = 1;//1;   //SPI Master mode 0= Input data sampled at middle of data output time
...
i =0;
	...
	write_spi_con(i,InitConfigRegsPer[i]);
		val = read_spi_con(i); 										//	__delay_ms(2);
												//	val = read_spi_con(2);
	  while (i < 0x20)
		  {
